<template>
   <div class="container">
       <topComponent title="确认信息">
           <span class="save" slot="right">取消上标</span>
       </topComponent>
       ...
       <div class="borrowInfoTit" v-if="!isOnce">
           <dl>
               <dt>
                   项目编号
                   <span class="col3">20151206100122</span>
               </dt>
               <dd>
                   借款期限(月）
           ...
           <ul>
               <li>
                   <b>2900</b>收款金额(元)<i class="queryIcon" @click="alertKnow('skje')">?</i>
               </li>
               <li>
                   <b>3000</b>授信金额(元)<i class="queryIcon" @click="alertKnow('sxje')">?</i>
               </li>
               <li>
                   <b>100</b>期初服务费(元)<i class="queryIcon" @click="alertKnow('qcfwf')">?</i>
               </li>
           </ul>
       </div>
       <ul class="listCom list-arrow list-mini no-top">
           <li class="mini" @click="$router.push('/loan/suremore')">更多详情</li>
       </ul>
       ...
       <div class="agreeMent mt20" :class="{no : !checked}" @click="checked = !checked">
           我已阅读并同意
           <span class="blue">《借款协议》</span>
           <span class="blue">《委托确认协议书》</span>
...
<script>
    ...
    export default {
        ...
        methods: {
            goSubmit:function () {
                if(this.checked == false) {
                    this.callDialog('请阅读并同意协议','sort',1200);
                }else {
        ...
        mounted:function() {
            if(Math.random()*10 >= 5)
                this.isOnce = false;    //随机显示一次性还款模板或等额模板
        }
    }
</script>
<style>
