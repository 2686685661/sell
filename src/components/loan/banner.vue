<template>
    <div class="indexBanner" id="banner" ref="banner">
        <ul ref="ul" style="margin-left: 0;">
            <li  v-for='data of lists'>
                <img :src='data.img' :alert='data.tit'>
            </li>
        </ul>
...
<script type="text/ecmascript">
    export default {
        data(){
            return {
                lists:[
                    {tit:'',img:'../../../static/img/songjiangdai.png',isOn:false,href:''},
                    {tit:'',img:'../../../static/img/zidonghuankuan.jpg',isOn:false,href:''}
                ],
            ...
            goAnimate(start,end){
                ...
                var gopo = setInterval(function(){
                    ...
                    if(b==time){
                        ...
                        that.$refs['ul'].style.marginLeft = end + 'px'
                    }else{
                        start = start + step; 	//路程堆积
                        that.$refs['ul'].style.marginLeft = start+'px'
                    }
                },m);
            }
        },
        mounted:function(){
            ...
            var that = this;	//that赋值,等同于this

            //获取DOM元素节点-[banner]-[span]+s
            var spans = document.getElementById('bannerTag').getElementsByTagName('span');
